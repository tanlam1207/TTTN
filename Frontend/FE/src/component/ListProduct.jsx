...
const fetchProductsByTagId = async (tagId) => {
  try {
...
const ListProduct = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const productIds = await fetchProductsByTagId(tagId);
        console.log('Product Ids:', productIds); // Kiểm tra productIds
        const productTagsResponse = await axios.get(
          "http://localhost:8080/api/products"
        );
        console.log('Product Tags Response:', productTagsResponse.data); // Kiểm tra dữ liệu trả về từ API
        const filteredProducts = productTagsResponse.data.filter((item) => productIds.includes(item.product_id));
        console.log('Filtered Products:', filteredProducts); // Kiểm tra filteredProducts
  ...
  console.log('====================================');
  console.log("list",listpro);
  console.log('====================================');
  if (loading) {
    return <div>Loading...</div>;
  } else {
    // Trả về UI cho listpro ở đây
  }
    ...
    <>
      <section className="section-content padding-y">
        <div className="container">
          ...
          <div className="row">
            {listpro.map((listpro) => (
              <div className="col-md-3" key={listpro.productId}>
                <figure className="card card-product-grid">
                  <div className="img-wrap">
                    <span className="badge badge-danger"> NEW </span>
                    <img src={`/asset/images/items/${listpro.sku}`} />
                  </div>{" "}
                  {/* img-wrap.// */}
                  <figcaption className="info-wrap">
                    <a href="#" className="title mb-2">
                      {listpro.product_name}a
                    </a>
